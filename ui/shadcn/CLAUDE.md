...
const ComponentTrigger = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => (
    <button ref={ref} className={cn("trigger-base", className)} {...props} />
  )
)
ComponentTrigger.displayName = "ComponentTrigger"

const ComponentContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("content-base", className)} {...props} />
  )
)
...
- **Local State**: Use `useState` for simple component-level interaction.
- **Compositional State**: Use Context API for deeply nested sub-components (e.g., Tabs, Accordion).
- **External State**: Use **Zustand** for global UI state (modals, sidebars, themes).
- **Form State**: Use **React Hook Form** with Zod validation.
- **Server State**: Use **TanStack Query** for fetching, caching, and synchronizing server data.
  - Implement `staleTime` (e.g., 5 min) to avoid redundant requests.
  - Use `useSuspenseQuery` for seamless integration with `<Suspense>`.

### 4. Visual Design & Micro-interactions

- **Consistency**: Always use Tailwind variables (e.g., `text-muted-foreground`, `bg-accent`).
- **Icons**: Use `Lucide React` for all icons.
- **Animations**: Use `framer-motion` for complex transitions or Tailwind's `transition-*` for simple ones.

### 5. Feedback & Loading Patterns

- **Feedback**: Use `Sonner` (Toast) for action confirmation.
- **Loading States**: Use `Skeleton` for placeholder content.
- **Streaming**: Wrap components using server data in `<Suspense fallback={<Skeleton />}>`.
...
Prefer **Compound Components** for complex UI parts to provide maximum flexibility:

```tsx
// Pattern: Tabs.List, Tabs.Trigger, Tabs.Content
const Parent = ({ children }) => <Context.Provider>{children}</Context.Provider>
const Child = () => {
...
### 8. Strict Form Patterns

Always use Zod for schema definition and `react-hook-form` for state:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ref={ref}
        className={cn("button-base-class", className)}
        {...props}
      />
    )
  }
...
// Focus trap in modals usage pattern
/*
<FocusTrap>
...
### 1. Tables with TanStack Table usage pattern
...
### 2. Code Splitting usage pattern
...
### 2. Visual Regression & Documentation

- **Storybook**: Use to document and test components in isolation.
- **Chromatic**: Use for automated visual regression testing (VRT).
- **A11y Plugin**: Use `@storybook/addon-a11y` for automated accessibility checks.
...
1. **Sanitize user input**: Always validate and sanitize user input before rendering, especially in `dangerouslySetInnerHTML`. Use **DOMPurify**.
2. **Form Validation**: Use Zod schemas with `react-hook-form` to ensure data integrity and prevent injection.
3. **Protected Components**: Implement "Authorized" wrappers for UI elements that should only be visible to specific roles.
4. **Secure Redirects**: Validate URLs before redirecting to avoid Open Redirect vulnerabilities.
5. **Clickjacking Protection**: Ensure `X-Frame-Options` is set to `DENY` or `SAMEORIGIN` in the framework configuration.
6. **Data Masking**: Mask sensitive information in UI (e.g., partial API keys, emails) using utility functions.
