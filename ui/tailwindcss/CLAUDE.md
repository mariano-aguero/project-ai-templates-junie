...
- **Logical Properties** for better i18n support (`ms-`, `me-`, `ps-`, `pe-`)
...
- **Spacing Scale**: Stick to Tailwind's 4-unit scale (0.25rem). Avoid arbitrary values like `mt-[13px]`.
- **Containers**: Always use a max-width container for page content to maintain readability.
- **Grids**: Use `grid` with `gap` for structured layouts instead of margins on children.

### 2. Consistency & Design Tokens
- **Colors**: Use semantic names when possible (`brand-primary`, `accent-success`).
- **Typography**: Maintain a clear hierarchy using `text-*` sizes and `font-*` weights.
- **Elevation**: Use a consistent shadow system (`shadow-sm`, `shadow-md`, etc.) to define depth.

### 3. Micro-interactions & Loading
- **Transitions**: Every interactive element should have a transition (e.g., `transition-colors duration-200`).
- **States**: Always define `:hover`, `:focus-visible`, and `:disabled` states.
- **Loading States**: Style `Skeleton` components using `animate-pulse` and consistent background colors (e.g., `bg-gray-200` or `bg-muted`).
- **Streaming**: Ensure layout stability when using `Suspense` to prevent layout shifts during content hydration.

### 4. Modular Responsibility
- **Encapsulation**: Components should manage their own internal spacing (gap/padding), but parent components should manage external spacing (margins) between children.
- **Container Queries**: Prefer `@container` for components that change layout based on their container size rather than the viewport size.
...
- Avoid using arbitrary values with user-provided data (e.g., `bg-[${userInput}]`). 
- Prefer predefined classes or style objects with strict validation.

### 2. Content Security Policy (CSP)
- When using Tailwind with CSP, avoid `unsafe-inline` if possible.
- If using JIT in production (rare for Next.js/Vite), ensure styles are extracted and not injected via script.

### 3. Responsive Layout Stability
- Use `aspect-ratio` to prevent layout shifts (CLS) which can be exploited for clickjacking/UI redressing.
- Always define dimensions for placeholders/skeletons.
